import logging


def is_sublist(a, b):
    """Return True if all elements of `a` occur in the same order in `b` (as full
    elements or substrings).

    Source:
    https://stackoverflow.com/questions/23667731/determine-if-all-elements-in-a-list-are-present-and-in-the-same-order-in-another
    """
    seq = iter(b)
    x = None
    try:
        for x in a:
            while x not in next(seq):
                pass
        else:
            return True
    except StopIteration:
        pass
    logging.error("Failed at '%s'", x)
    return False


log_test_simple_initial_setup = [
    "Cat becomes active",
    "Cat attacks Dog",
    "Dog gets attacked by Cat",
    "Dog new health: 4",
    "Dog becomes active",
    "Dog attacks Cat",
    "Cat gets attacked by Dog",
    "Cat new health: 1",
    "Preparing Steed, moving to computer line",
    "Steed becomes active",
    "Player Schnuzgi loses 2 health, new health 3",
    "Cat becomes active",
    "Cat attacks Dog",
    "Dog gets attacked by Cat",
    "Dog new health: 3",
    "Dog becomes active",
    "Dog attacks Cat",
    "Cat gets attacked by Dog",
    "Cat dies.",
    "Removed card from GridPos(line=2, slot=0)",
    "Steed becomes active",
    "Player Schnuzgi loses 2 health, new health 1",
    "Dog becomes active",
    "Player Schnuzgi loses 2 health, new health -1",
    "Steed becomes active",
    "Player Schnuzgi loses 2 health, new health -3",
    "Player Schnuzgi loses 1 life, 0 life/lives left, 3 overflow damage",
]

log_test_human_decks_managed_correctly = [
    "Handdeck size: 4 (Koala,Weasel,Lynx,Hamster)",
    "Fightdeck size: 1 (Porcupine)",
    "Hamsterdeck size: 9 (Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Useddeck size: 0 ()",
    "Human draws Hamster",
    "Human plays Koala on 0",
    "Handdeck size: 4 (Weasel,Lynx,Hamster,Hamster)",
    "Fightdeck size: 1 (Porcupine)",
    "Hamsterdeck size: 8 (Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Useddeck size: 1 (Koala)",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: Card(name='Koala', initial_power=1, initial_health=3, power=1, health=3, skills=[]), None, None, None",
    "Koala becomes active",
    "Koala attacks Hulk",
    "Hulk gets attacked by Koala",
    "Hulk new health: 99",
    "Hulk becomes active",
    "Hulk attacks Koala",
    "Koala gets attacked by Hulk",
    "Koala new health: 1",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 98",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 96",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 94",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: Card(name='Koala', initial_power=1, initial_health=3, power=1, health=1, skills=[]), None, None, None",
    "Handdeck size: 4 (Weasel,Lynx,Hamster,Hamster)",
    "Fightdeck size: 1 (Porcupine)",
    "Hamsterdeck size: 8 (Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Useddeck size: 1 (Koala)",
    "Human draws Porcupine",
    "Human plays Weasel on 1",
    "Handdeck size: 4 (Lynx,Hamster,Hamster,Porcupine)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 8 (Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Useddeck size: 2 (Koala,Weasel)",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: Card(name='Koala', initial_power=1, initial_health=3, power=1, health=1, skills=[]), Card(name='Weasel', initial_power=1, initial_health=1, power=1, health=1, skills=[]), None, None",
    "Koala becomes active",
    "Koala attacks Hulk",
    "Hulk gets attacked by Koala",
    "Hulk new health: 98",
    "Weasel becomes active",
    "Weasel attacks Hulk",
    "Hulk gets attacked by Weasel",
    "Hulk new health: 99",
    "Hulk becomes active",
    "Hulk attacks Koala",
    "Koala gets attacked by Hulk",
    "Koala dies.",
    "Removed card from GridPos(line=2, slot=0)",
    "Hulk becomes active",
    "Hulk attacks Weasel",
    "Weasel gets attacked by Hulk",
    "Weasel dies.",
    "Removed card from GridPos(line=2, slot=1)",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 92",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 90",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=98, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: None, None, None, None",
    "Handdeck size: 4 (Lynx,Hamster,Hamster,Porcupine)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 8 (Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Useddeck size: 2 (Koala,Weasel)",
    "Human draws Hamster",
    "Human plays Lynx on 0",
    "Handdeck size: 4 (Hamster,Hamster,Porcupine,Hamster)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 7 (Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Useddeck size: 3 (Koala,Weasel,Lynx)",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=98, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: Card(name='Lynx', initial_power=3, initial_health=2, power=3, health=2, skills=[]), None, None, None",
    "Lynx becomes active",
    "Lynx attacks Hulk",
    "Hulk gets attacked by Lynx",
    "Hulk new health: 95",
    "Hulk becomes active",
    "Hulk attacks Lynx",
    "Lynx gets attacked by Hulk",
    "Lynx dies.",
    "Removed card from GridPos(line=2, slot=0)",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 88",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 86",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 84",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=95, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: None, None, None, None",
    "Handdeck size: 4 (Hamster,Hamster,Porcupine,Hamster)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 7 (Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Useddeck size: 3 (Koala,Weasel,Lynx)",
    "Human draws Hamster",
    "Human plays Hamster on 0",
    "Handdeck size: 4 (Hamster,Porcupine,Hamster,Hamster)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 6 (Hamster,Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Useddeck size: 4 (Koala,Weasel,Lynx,Hamster)",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=95, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: Card(name='Hamster', initial_power=0, initial_health=1, power=0, health=1, skills=[]), None, None, None",
    "Hamster becomes active",
    "Hamster would attack but has 0 power, so doesn't",
    "Hulk becomes active",
    "Hulk attacks Hamster",
    "Hamster gets attacked by Hulk",
    "Hamster dies.",
    "Removed card from GridPos(line=2, slot=0)",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 82",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 80",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 78",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=95, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: None, None, None, None",
    "Handdeck size: 4 (Hamster,Porcupine,Hamster,Hamster)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 6 (Hamster,Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Useddeck size: 4 (Koala,Weasel,Lynx,Hamster)",
    "Human draws Hamster",
    "Human plays Hamster on 0",
    "Handdeck size: 4 (Porcupine,Hamster,Hamster,Hamster)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 5 (Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Useddeck size: 5 (Koala,Weasel,Lynx,Hamster,Hamster)",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=95, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: Card(name='Hamster', initial_power=0, initial_health=1, power=0, health=1, skills=[]), None, None, None",
    "Hamster becomes active",
    "Hamster would attack but has 0 power, so doesn't",
    "Hulk becomes active",
    "Hulk attacks Hamster",
    "Hamster gets attacked by Hulk",
    "Hamster dies.",
    "Removed card from GridPos(line=2, slot=0)",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 76",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 74",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 72",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=95, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: None, None, None, None",
    "Handdeck size: 4 (Porcupine,Hamster,Hamster,Hamster)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 5 (Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Useddeck size: 5 (Koala,Weasel,Lynx,Hamster,Hamster)",
    "Human draws Hamster",
    "Human plays Porcupine on 0",
    "Handdeck size: 4 (Hamster,Hamster,Hamster,Hamster)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 4 (Hamster,Hamster,Hamster,Hamster)",
    "Useddeck size: 6 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine)",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=95, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: Card(name='Porcupine', initial_power=1, initial_health=2, power=1, health=2, skills=[<Skill.AIRDEFENSE: SkillSpec(name='Air Defense', symbol='🚀', description='A card with Air Defense blocks opposing Soaring creatures.')>]), None, None, None",
    "Porcupine becomes active",
    "Porcupine🚀 attacks Hulk",
    "Hulk gets attacked by Porcupine",
    "Hulk new health: 94",
    "Hulk becomes active",
    "Hulk attacks Porcupine 🚀",
    "Porcupine gets attacked by Hulk",
    "Porcupine dies.",
    "Removed card from GridPos(line=2, slot=0)",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 70",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 68",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 66",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=94, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: None, None, None, None",
    "Handdeck size: 4 (Hamster,Hamster,Hamster,Hamster)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 4 (Hamster,Hamster,Hamster,Hamster)",
    "Useddeck size: 6 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine)",
    "Human draws Hamster",
    "Human plays Hamster on 0",
    "Handdeck size: 4 (Hamster,Hamster,Hamster,Hamster)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 3 (Hamster,Hamster,Hamster)",
    "Useddeck size: 7 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine,Hamster)",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=94, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: Card(name='Hamster', initial_power=0, initial_health=1, power=0, health=1, skills=[]), None, None, None",
    "Hamster becomes active",
    "Hamster would attack but has 0 power, so doesn't",
    "Hulk becomes active",
    "Hulk attacks Hamster",
    "Hamster gets attacked by Hulk",
    "Hamster dies.",
    "Removed card from GridPos(line=2, slot=0)",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 64",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 62",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 60",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=94, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: None, None, None, None",
    "Handdeck size: 4 (Hamster,Hamster,Hamster,Hamster)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 3 (Hamster,Hamster,Hamster)",
    "Useddeck size: 7 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine,Hamster)",
    "Human draws Hamster",
    "Human plays Hamster on 0",
    "Handdeck size: 4 (Hamster,Hamster,Hamster,Hamster)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 2 (Hamster,Hamster)",
    "Useddeck size: 8 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine,Hamster,Hamster)",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=94, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: Card(name='Hamster', initial_power=0, initial_health=1, power=0, health=1, skills=[]), None, None, None",
    "Hamster becomes active",
    "Hamster would attack but has 0 power, so doesn't",
    "Hulk becomes active",
    "Hulk attacks Hamster",
    "Hamster gets attacked by Hulk",
    "Hamster dies.",
    "Removed card from GridPos(line=2, slot=0)",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 58",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 56",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 54",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=94, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: None, None, None, None",
    "Handdeck size: 4 (Hamster,Hamster,Hamster,Hamster)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 2 (Hamster,Hamster)",
    "Useddeck size: 8 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine,Hamster,Hamster)",
    "Human draws Hamster",
    "Human plays Hamster on 0",
    "Handdeck size: 4 (Hamster,Hamster,Hamster,Hamster)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 1 (Hamster)",
    "Useddeck size: 9 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine,Hamster,Hamster,Hamster)",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=94, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: Card(name='Hamster', initial_power=0, initial_health=1, power=0, health=1, skills=[]), None, None, None",
    "Hamster becomes active",
    "Hamster would attack but has 0 power, so doesn't",
    "Hulk becomes active",
    "Hulk attacks Hamster",
    "Hamster gets attacked by Hulk",
    "Hamster dies.",
    "Removed card from GridPos(line=2, slot=0)",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 52",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 50",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 48",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=94, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: None, None, None, None",
    "Handdeck size: 4 (Hamster,Hamster,Hamster,Hamster)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 1 (Hamster)",
    "Useddeck size: 9 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine,Hamster,Hamster,Hamster)",
    "Human draws Hamster",
    "Human plays Hamster on 0",
    "Handdeck size: 4 (Hamster,Hamster,Hamster,Hamster)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 0 ()",
    "Useddeck size: 10 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine,Hamster,Hamster,Hamster,Hamster)",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=94, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: Card(name='Hamster', initial_power=0, initial_health=1, power=0, health=1, skills=[]), None, None, None",
    "Hamster becomes active",
    "Hamster would attack but has 0 power, so doesn't",
    "Hulk becomes active",
    "Hulk attacks Hamster",
    "Hamster gets attacked by Hulk",
    "Hamster dies.",
    "Removed card from GridPos(line=2, slot=0)",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 46",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 44",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 42",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=94, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: None, None, None, None",
    "Handdeck size: 4 (Hamster,Hamster,Hamster,Hamster)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 0 ()",
    "Useddeck size: 10 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine,Hamster,Hamster,Hamster,Hamster)",
    "Human plays Hamster on 0",
    "Handdeck size: 3 (Hamster,Hamster,Hamster)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 0 ()",
    "Useddeck size: 11 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine,Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=94, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: Card(name='Hamster', initial_power=0, initial_health=1, power=0, health=1, skills=[]), None, None, None",
    "Hamster becomes active",
    "Hamster would attack but has 0 power, so doesn't",
    "Hulk becomes active",
    "Hulk attacks Hamster",
    "Hamster gets attacked by Hulk",
    "Hamster dies.",
    "Removed card from GridPos(line=2, slot=0)",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 40",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 38",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 36",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=94, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: None, None, None, None",
    "Handdeck size: 3 (Hamster,Hamster,Hamster)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 0 ()",
    "Useddeck size: 11 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine,Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Human plays Hamster on 0",
    "Handdeck size: 2 (Hamster,Hamster)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 0 ()",
    "Useddeck size: 12 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=94, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: Card(name='Hamster', initial_power=0, initial_health=1, power=0, health=1, skills=[]), None, None, None",
    "Hamster becomes active",
    "Hamster would attack but has 0 power, so doesn't",
    "Hulk becomes active",
    "Hulk attacks Hamster",
    "Hamster gets attacked by Hulk",
    "Hamster dies.",
    "Removed card from GridPos(line=2, slot=0)",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 34",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 32",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 30",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=94, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: None, None, None, None",
    "Handdeck size: 2 (Hamster,Hamster)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 0 ()",
    "Useddeck size: 12 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Human plays Hamster on 0",
    "Handdeck size: 1 (Hamster)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 0 ()",
    "Useddeck size: 13 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=94, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: Card(name='Hamster', initial_power=0, initial_health=1, power=0, health=1, skills=[]), None, None, None",
    "Hamster becomes active",
    "Hamster would attack but has 0 power, so doesn't",
    "Hulk becomes active",
    "Hulk attacks Hamster",
    "Hamster gets attacked by Hulk",
    "Hamster dies.",
    "Removed card from GridPos(line=2, slot=0)",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 28",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 26",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 24",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=94, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: None, None, None, None",
    "Handdeck size: 1 (Hamster)",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 0 ()",
    "Useddeck size: 13 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Human plays Hamster on 0",
    "Handdeck size: 0 ()",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 0 ()",
    "Useddeck size: 14 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=94, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: Card(name='Hamster', initial_power=0, initial_health=1, power=0, health=1, skills=[]), None, None, None",
    "Hamster becomes active",
    "Hamster would attack but has 0 power, so doesn't",
    "Hulk becomes active",
    "Hulk attacks Hamster",
    "Hamster gets attacked by Hulk",
    "Hamster dies.",
    "Removed card from GridPos(line=2, slot=0)",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 22",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 20",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 18",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=94, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: None, None, None, None",
    "Handdeck size: 0 ()",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 0 ()",
    "Useddeck size: 14 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Handdeck size: 0 ()",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 0 ()",
    "Useddeck size: 14 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=94, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: None, None, None, None",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 16",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 14",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 12",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 10",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=94, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: None, None, None, None",
    "Handdeck size: 0 ()",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 0 ()",
    "Useddeck size: 14 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Handdeck size: 0 ()",
    "Fightdeck size: 0 ()",
    "Useddeck size: 14 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=94, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: None, None, None, None",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 8",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 6",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 4",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 2",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=94, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: None, None, None, None",
    "Handdeck size: 0 ()",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 0 ()",
    "Useddeck size: 14 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Handdeck size: 0 ()",
    "Fightdeck size: 0 ()",
    "Hamsterdeck size: 0 ()",
    "Useddeck size: 14 (Koala,Weasel,Lynx,Hamster,Hamster,Porcupine,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster,Hamster)",
    "Grid line 0: None, None, None, None",
    "Grid line 1: Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=94, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=99, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[]), Card(name='Hulk', initial_power=2, initial_health=100, power=2, health=100, skills=[])",
    "Grid line 2: None, None, None, None",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health 0",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health -2",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health -4",
    "Hulk becomes active",
    "Player Schnuzgi loses 2 health, new health -6",
    "Player Schnuzgi loses 1 life, 0 life/lives left, 6 overflow damage",
]
